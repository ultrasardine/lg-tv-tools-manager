...
This module provides LG TV discovery using SSDP multicast. It sends
M-SEARCH requests and collects responses from UPnP devices, filtering
for LG TVs based on manufacturer and device information.
"""
...
# Multiple search targets to maximize discovery chances.
...
def _parse_headers(payload: str) -> dict[str, str]:
    """Parse SSDP response headers into a dictionary."""
...
def _manufacturer_filter_enabled() -> bool:
    """Check if strict manufacturer filtering is enabled via environment."""
...
def _get_local_ips() -> list[str]:
    ...
    Uses netifaces if available, falls back to socket connect trick.
    """
    ips: list[str] = []
...
def discover_lg_tvs(timeout: float = 5.0) -> list[LGTVDevice]:
    ...
    # Send M-SEARCH packets multiple times for reliability
    send_failed = True
    ...
    try:
        while time.time() < end:
            ...
            # Skip responses without a location
            if not location:
                continue
            ...
            # Only add devices confirmed as LG
    ...
    LOGGER.info(
        "SSDP discovered %d logical and %d physical LG devices",
        len(logical_devices),
