...
class _LGServiceListener:
    ...
    def _handle_service(self, zc: Zeroconf, service_type: str, name: str) -> None:
        ...
        _serial = self._extract_text_property(properties, "serialNumber")  # Reserved for future use

        # Build a friendly name from the service instance name
        # e.g. "[LG] TV3 UA731C0LA._airplay._tcp.local." -> "[LG] TV3 UA731C0LA"
        friendly_name = name.replace(f".{service_type}", "").strip(".")

        # Use IP as the deduplication key (multiple services from same TV)
